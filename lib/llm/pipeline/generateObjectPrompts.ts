...
const CLUE_OBJECT_PROMPT_REQUIREMENTS = [
  ...
  "standalone prop for compositing into a room later, not a pre-composed scene",
  "slightly isometric top-front view, around 45 degree camera angle",
  "bottom-center anchor feeling",
  "whole object fully visible",
  "object occupies about 55 to 70 percent of the square canvas with generous empty margins",
  "transparent PNG background if supported, otherwise perfectly uniform plain background for easy cutout",
  "grounded silhouette with a subtle contact shadow directly under the object only",
  "realistic macro photography, highly detailed",
  ...
  "no walls, no floor environment, no diorama box",
  "no furniture around it",
  "no tabletop scene",
  "no framed panel",
  "no rectangular card composition",
  "no wide horizontal crop",
  "no screenshot layout",
  "no cropped close-up",
  "no multiple objects",
  "no flat sticker",
  "no flat 2D",
...
const PET_SPRITE_PROMPT_REQUIREMENTS = [
  ...
  "slightly isometric top-front view, around 45 degree camera angle",
  "bottom-center anchor feeling",
  "whole pet fully visible",
  "pet occupies about 50 to 65 percent of the square canvas with generous empty margins",
  "transparent PNG background if supported, otherwise perfectly uniform plain background for easy cutout",
  "grounded silhouette with a subtle contact shadow directly under the pet only",
  "realistic macro photography, highly detailed",
...
const DEFAULT_NEGATIVE_PROMPT = [
  "room, walls, floor, environment, scenery, background scene, diorama box",
  "no flat sticker",
  "no flat 2D",
  "no decal",
  "no clip art",
...
const IMAGE_PROMPT_SYSTEM_PROMPT = [
  ...
  "clue_object_sprite 和 pet_sprite 必须优先面向透明底 PNG 来写 prompt；如果模型不支持透明底，也必须要求纯净单色底和完整物件留白，方便后续抠图。",
